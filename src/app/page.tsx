...
export default function Home() {
  ...
  return (
    <div className="mx-auto flex h-screen w-full flex-col items-center justify-center gap-[20px] md:gap-[40px]">
      <Header />
      <CalendarHeader
      ...
      <Calendar
        ...
        type={"week"}
      />
      <div className="h-[400px] flex flex-col justify-between items-center gap-[30px]">
        <div className="relative flex h-[120px] w-[400px] items-center justify-center rounded-2xl border-[1px] bg-primary-white p-4 text-primary-black shadow-lg">
          ...
          {monthData && monthData.attendance.length > 0 ? (
            <p className="text-2xl font-bold">
              {seolgiSay === 'month' ? `${monthData.month}월의 설기 개수는 ${monthData.attendance.length}
              개입니다!`
                :seolgiSay === 'week' ? `이번 주의 설기 개수는 0개 입니다!`
                :seolgiSay === 'objective' ? `이번 목표의 설기 개수는 10개 입니다!`
                :null }
            </p>
          ) : (
        ...
        <div className={`${seolgiSize[seolgiSay]} transition-all ${
            effect && "animate-seolgiClick"
          }`}
          onClick={() => {
            seolgiSay === 'objective' ? setSeolgiSay('month')
            :seolgiSay === 'month' ? setSeolgiSay('week')
            :seolgiSay === 'week' ? setSeolgiSay('objective') : null
            setEffect(true);
          }}
