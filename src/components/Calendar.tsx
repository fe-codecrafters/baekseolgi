...
const Day = ({ day, type, isThisMonth }) => {
  ...
  return (
    <div className="mx-1 flex flex-col items-center">
      <div
        className={`h-[29px] w-[70px] text-center${
          type === "day" ? " mt-[20px] text-[24px]" : " mt-[14px] text-[16px]"
        }${isToday ? " font-medium text-black" : " text-neutral-500"}`}
      >
        {day.id ? day.id : day !== 0 ? day : null}
      </div>
      {type === "date" ? (
        <div className={`mt-[6px] h-[70px] w-[70px] rounded-[10px] ${bgColor}`}>
          {day.id ? (
...
const Week = ({ week, type, isThisMonth }) => {
  return (
    <div className="weekly-row mt-1 flex w-auto flex-row justify-start">
      {week.map((day) => (
        <Day day={day} type={type} isThisMonth={isThisMonth} />
      ))}
    </div>
  );
};
...
export const Calendar = ({ type }) => {
  ...
  return (
    <div className="calendar flex flex-col items-center">
      ...
      {weeks.map((week) => (
        <Week week={week} type={"date"} isThisMonth={isThisMonth} />
      ))}
    </div>
  );
};
